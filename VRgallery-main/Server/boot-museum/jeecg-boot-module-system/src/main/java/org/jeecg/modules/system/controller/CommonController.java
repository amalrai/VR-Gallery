...
public class CommonController {
    ...
    @Autowired
    private ISysLanguageItemService languageItemService;
    ...
    public Result<?> upload(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(CommonConstant.UPLOAD_TYPE_LOCAL.equals(uploadType)){
            ...
            */
            ...
            savePath = CommonUtils.upload(file, bizPath, uploadType, isEncode);
            //update-end-author:taoyan date:20200814 for:文件上传改造
        }
        if(oConvertUtils.isNotEmpty(savePath)){
            result.setSuccess(true);
            try {
                CommonFileVo commonFileVo = JSON.parseObject(savePath, CommonFileVo.class);
...
//	}
    ...
    @RequestMapping("/getLanguageDataByKey")
    public Result<?> getLanguageDataByKey(@RequestParam("languageCode") String languageCode, HttpServletRequest request) {
        try {
            ...
            return languageItemService.getDataByKey(languageCode, userName);
        } catch (Exception e) {
            log.error("获取多语言配置失败", e);
