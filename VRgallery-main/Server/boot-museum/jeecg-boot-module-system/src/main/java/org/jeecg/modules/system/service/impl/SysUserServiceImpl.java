...
//		this.save(user);  //保存角色的时候已经添加过一次了
	...
	public Result<?> checkUserIsEffective(SysUser sysUser) {
		...
		//情况1：根据用户信息查询，该用户不存在
		if (sysUser == null) {
			result.error500(MessageUtil.getMessage("该用户不存在，请注册", UserLocalContext.getLocal()));
		...
		//update-end---author:王帅   Date:20200601  for：if条件永远为falsebug------------
			...
			result.error500(MessageUtil.getMessage("该用户已注销", UserLocalContext.getLocal()));
			return result;
		}
		//情况3：根据用户信息查询，该用户已冻结
		if (CommonConstant.USER_FREEZE.equals(sysUser.getStatus())) {
			baseCommonService.addLog("用户登录失败，用户名:" + sysUser.getUsername() + "已冻结！", CommonConstant.LOG_TYPE_1, null);
			result.error500(MessageUtil.getMessage("该用户已冻结", UserLocalContext.getLocal()));
			return result;
		}
		return result;
	}
