package org.jeecg.modules.show.administrator.controller;
...
@Slf4j
@RestController
@RequestMapping("/show/administrator")
public class AdministratorController {
	...
	public Result<IPage<SysUser>> queryPageList(SysUser user,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        queryWrapper.isNotNull("show_museum_id");
	...
	public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
		...
		try {
			...
			//查询博物馆角色的ID
            SysRole role = sysRoleService.getBaseMapper().selectOne(new QueryWrapper<SysRole>().lambda().eq(SysRole::getRoleCode, "museum"));
            if(role == null){
                result.error500("操作失败");
                return result;
            }
			// 保存用户走一个service 保证事务
			sysUserService.saveUser(user, role.getId(), selectedDeparts);
			result.success("添加成功！");
		} catch (Exception e) {
...
//				String roles = jsonObject.getString("selectedroles");
                ...
                // 修改用户走一个service 保证事务
				sysUserService.editUser(user, role.getId(), departs);
				result.success("修改成功!");
			}
		} catch (Exception e) {
			log.error(e.getMessage(), e);
