  ...
  export default {
    ...
    data() {
      return {
        ...
        menuVisible: false,
        menuItemList: [
          { key: '4', icon: 'reload', text: this.$t('刷 新') },
          { key: '1', icon: 'arrow-left', text: this.$t('关闭左侧') },
          { key: '2', icon: 'arrow-right', text: this.$t('关闭右侧') },
          { key: '3', icon: 'close', text: this.$t('关闭其它') }
        ],
        reloadFlag:true
      }
    },
    ...
    methods: {
      ...
      changeTitle(title) {
        let projectTitle = this.$t('sys.name');
        // 首页特殊处理
        if (this.$route.path === indexKey) {
          document.title = projectTitle
        } else {
          document.title = this.$t(title) + ' · ' + projectTitle
        }
      },
      ...
      remove(key) {
        if (key == indexKey) {
          this.$message.warning(this.$t('首页不能关闭!'))
          return
        }
        if (this.pageList.length === 1) {
          this.$message.warning(this.$t('这是最后一页，不能再关闭了啦'))
          return
        }
