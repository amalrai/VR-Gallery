...
<script>
  ...
  export default {
    ...
    created(){
      ...
      if(this.action){
        this.uploadAction = window._CONFIG['domianURL'] + this.action
      }
      //---------------------------- end 图片左右换位置 -------------------------------------
    },
    ...
    methods:{
      ...
      handlePathChange(){
        ...
        for(var a=0;a<uploadFiles.length;a++){
          ...
          if(uploadFiles[a].status === 'done' && uploadFiles[a].response.success) {
      ...
      doBeforeUpload(file) {
        ...
        return new Promise((resolve, reject) => {
          ...
          if(that.accept){
            isAccept = false;
            let acceptArr = that.accept.split(",");
            for (let i = 0; i < acceptArr.length; i++) {
              if(file.type === acceptArr[i]){
                isAccept = true;
              }
            }
            if (!isAccept) {
          ...
          if(that.limitHeight && that.limitWidth > 0 && file.type.indexOf("video")>=0){
            const video = document.createElement('video')
            video.addEventListener('loadedmetadata', event => {
              if(video.videoHeight > that.limitHeight || video.videoWidth > that.limitWidth){
            ...
            return resolve(isAccept && isLt1M);
          }
        })
      },
      doAfterUpload(response){
          ...
          if(info.file.response.success){
            ...
            fileList = fileList.map((file, index) => {
              if (file.response.success) {
                ...
                errMsg += file.name + file.response.message;
                errIndexs.push(index);
              }
            });
            for (let errIndex of errIndexs) {
              fileList.splice(errIndex, 1);
            }
            if(errMsg !== ""){
