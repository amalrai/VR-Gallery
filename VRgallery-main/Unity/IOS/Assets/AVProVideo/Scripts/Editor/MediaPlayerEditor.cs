	...
	public class MediaPlayerEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			// Display the file name and buttons to load new files
			{
				...
				if (Application.isPlaying)
				{
					...
					{
						media.OpenVideoFromFile(media.m_VideoLocation, media.m_VideoPath);
					}
				}
		...
		private void OnInspectorGUI_About()
		{
			...
			if (_expandAbout)
			{
				...
				{
					CustomUrlOpener.Open(LinkUserManual);
				}
				if (GUILayout.Button("Scripting Class Reference", GUILayout.ExpandWidth(false)))
				{
					CustomUrlOpener.Open(LinkScriptingClassReference);
				}
				...
				{
					CustomUrlOpener.Open(LinkAssetStorePage);
				}
				...
				{
					CustomUrlOpener.Open(LinkForumPage);
				}
				...
				{
					CustomUrlOpener.Open(LinkPluginWebsite);
				}
		...
		private static void GetRelativeLocationFromPath(string path, ref string filePath, ref MediaPlayer.FileLocation fileLocation)
		{
			...
			else
			{
				...
				if (path.StartsWith(Application.persistentDataPath + "/1"))
				{
					filePath = GetPathRelativeTo(Application.persistentDataPath + "/1", path);
