...
public class DragRotate : MonoBehaviour
{
    ...
    public Button fileDetectionButton;

    public long wenjiandaxiao;
    ...
    public void CEshjoi()
    {
        ...
        for (int i = 0; i < urls.Length; i++)
        {
            ...
            if (Directory.Exists(ur))
            {
                ...
                DirectoryInfo direction = new DirectoryInfo(ur);
                FileInfo[] files = direction.GetFiles("*", SearchOption.AllDirectories);
                Debug.Log(files.Length / 2);
                frames.m_frameSprite = new Texture[files.Length / 2];
                frames.fileNames = new string[files.Length / 2];
    ...
    private void ContentLoad(string path)
    {
        ...
        //                string ur = "jar:file://" + Application.dataPath + "!/assets/" + path;
        //#endif

        //#if UNITY_EDITOR
        //        string ur = Application.persistentDataPath + "/1" + "/" + path;
        //#elif UNITY_ANDROID
        //        string ur = "file://" + Application.persistentDataPath + "/1" + "/" + path;
        //#endif

        for (int l = 0; l < frameSprites[frameIndex].m_frameSprite.Length; l++)
        {
            if (frameSprites[frameIndex].m_frameSprite[l] == null)
            {
                Debug.Log("图片为空，进行加载");
                break;
            ...
            else
            {
                ...
                m_rotateCoefficient = 360f / frameSprites[frameIndex].m_frameSprite.Length;
        ...
        for (int l = 0; l < frameSprites[frameIndex].m_frameSprite.Length; l++)
        {
            ...
            //    //}
            //    Debug.Log("c");
            //    Debug.Log(texdl.texture.name);

            //    //Texture2D t = texdl.texture;

            //    Texture2D t = new Texture2D(texdl.texture.width,texdl.texture.height,TextureFormat.DXT1,false);
            //    Debug.Log(t.format);
            //    t = texdl.texture;
            //    Debug.Log(t.format);

            //    //wr.downloadHandler.Dispose();
            //    //wr.Dispose();

            //    frameSprites[frameIndex].m_frameSprite[l] = t;
            //    Debug.Log(frameSprites[frameIndex].m_frameSprite[l].name);
            //    if (l == frameSprites[frameIndex].fileNames.Length - 1)
            //    {
            //        m_isRotate = true;
            ...
            //Texture2D texture1 = new Texture2D(1920, 1080);
            //texture1.LoadImage(bytes);
            //Texture2D texture2 = new Texture2D(width, height);
            //for (int i = 0; i < texture1.height; i++)
            //{
            //    for (int j = 0; j < texture1.width; j++)
            //    {
            //        Color color = texture1.GetPixel(j*10,i*10);
            //        texture2.SetPixel(j,i,color);
            //    }
            //}
            //texture2.Apply();
            //frameSprites[frameIndex].m_frameSprite[l] = texture2;
            ...
            Debug.Log(texture1.format);

            texture1.LoadImage(bytes);
            //texture1.LoadRawTextureData(bytes);

            Debug.Log(texture1.format);

            Debug.Log(texture1.graphicsFormat);

            texture1.Apply(false, true);

            //texture1.Compress(false);
        ...
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].height);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].width);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].filterMode);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].graphicsFormat);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].mipMapBias);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].mipmapCount);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].wrapMode);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].anisoLevel);
        Debug.Log(frameSprites[frameIndex].m_frameSprite[0].texelSize);
    ...
    void Update()
    {
        ...
        //if (m_Cube.transform.localEulerAngles.y >= 359)
        //    eulerindex = frameSprites[frameIndex].m_frameSprite.Length - 1;
        if (eulerindex > frameSprites[frameIndex].m_frameSprite.Length - 1)
            eulerindex = frameSprites[frameIndex].m_frameSprite.Length - 1;
        Debug.Log("(int)y:" + (int)m_Cube.transform.localEulerAngles.y);
        Debug.Log("(int)rotateCoefficient:" + (int)m_rotateCoefficient);
        Debug.Log("eulerindex:" + eulerindex);
    ...
    public void ToggleSpritesArray()
    {
        ...
        if (currentIndex.Count >= 5)
        {
            ...
            for (int j = 0; j < currentIndex.Count; j++)
            {
                if (j == currentIndex.Count - 1)
                {
                    if (frameIndex == currentIndex[j])
                    {
                        break;
                    }
                    else
                    {
                        ...
                        {
                            Destroy(frameSprites[currentIndex[0]].m_frameSprite[i]);
                            frameSprites[currentIndex[0]].m_frameSprite[i] = null;
                        }
                        ...
                        {
                            if (i == currentIndex.Count - 1)
                            {
                                currentIndex[i] = frameIndex;
                            }
                            else
                            {
                                int d = currentIndex[i + 1];
                                currentIndex[i] = d;
                            }
                        }
                    }
            ...
            for (int j = 0; j < currentIndex.Count; j++)
            {
                if (j == currentIndex.Count - 1)
                {
                    ...
                    else
                    {
                        for (int i = 0; i < frameSprites.Count; i++)
                        {
                            Destroy(frameSprites[currentIndex[currentIndex.Count - 1]].m_frameSprite[i]);
                            frameSprites[currentIndex[currentIndex.Count - 1]].m_frameSprite[i] = null;
                        }

                        Resources.UnloadUnusedAssets();
                        System.GC.Collect();

                        for (int i = currentIndex.Count - 1; i >= 0; i--)
                        {
                            if (i == 0)
                            {
                                currentIndex[i] = frameIndex;
    ...
    {
        int eulerindex = (int)m_Cube.transform.localEulerAngles.y / (int)m_rotateCoefficient;
        //frameTexture.sprite = frameSprites[frameIndex].m_frameSprite[eulerindex];
        frameTexture.SetTexture("_MainTex", frameSprites[frameIndex].m_frameSprite[eulerindex]);
    }
    ...
    {
        fileDetectionButton.interactable = false;
        string ur = "";
#if UNITY_EDITOR
        ur = Application.streamingAssetsPath + "/";
#elif UNITY_ANDROID
       ur = "jar:file://" + Application.dataPath + "!/assets/";
#elif UNITY_IOS|| UNITY_IPHONE
       ur = "file://" + Application.dataPath + "!/assets/";
#endif
        for (int i = 0; i < frameSprites.Count; i++)
        {
